...
爬虫学习资料夹：用 **`urllib` / `requests` / `BeautifulSoup` / `lxml(xpath)` / `jsonpath` / `selenium` / `scrapy`** 把常见抓取场景都跑了一遍，并配了部分本地样例文件（HTML/JSON/图片）。
...
### 2.1 坑 1：requests 的 GET 参数要用 `params`
...
## 1 scrapy_baidu（多 spider + 管道 + 图片下载）
...
## 2 scrapy_dushu（CrawlSpider + Rule）
